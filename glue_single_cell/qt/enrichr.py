...
class EnrichpyDialog(QtWidgets.QDialog):
    ...
    def _apply(self, do_dialog=True):
        ...
        ]  # [x.upper() for x in gene_list] Maybe just for humans?
        output_df = enrichrpy.enrichr.get_pathway_enrichment(
            gene_list_upper, gene_set_library=self.state.gene_set
        )
        output_df["Overlapping genes"] = output_df["Overlapping genes"].apply(
            lambda x: "|".join(x)
        )
