...
class QTLLayerArtist(ScatterLayerArtist):
    ...
    def __init__(self, axes, viewer_state, layer_state=None, layer=None):
        ...
        super(QTLLayerArtist, self).__init__(
            axes, viewer_state, layer_state=layer_state, layer=layer
        )
    ...
    def _update_data(self):
        ...
        if (
            self._viewer_state.lod_att is not None
            and self._viewer_state.lod_thresh is not None
        ):
            # print("Applying lod thresholding...")
            try:
                if not self.state.density_map:
                    lod = ensure_numerical(
                        self.layer[self._viewer_state.lod_att].ravel()
                    )
            except (IncompatibleAttribute, IndexError):
                # The following includes a call to self.clear()
            ...
            if self.state.density_map:
                ...
                if force or "stretch" in changed:
                    self.density_artist.set_norm(
                        ImageNormalize(stretch=STRETCHES[self.state.stretch]())
                    )
